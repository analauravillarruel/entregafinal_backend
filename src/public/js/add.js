...
addProductForm.addEventListener('submit', async (event) => {
  ...
  const formData = new FormData(addProductForm);
  const thumbnails = getThumbnails(thumbnailsInput.value);
  const product = Object.fromEntries(formData.entries());
  const newProduct = {
      ...product,
...
deleteButtons.forEach(button => {
  button.addEventListener('click', () => {
    ...
    deleteProductOnClient(productId);
  });
});
  ...
  try {
      ...
      if (response.ok) {
          // Si la respuesta es exitosa, podrías actualizar la lista de productos o hacer algo más
      } else {
